...
if [[ "$REBUILD_ENV" =~ ^[Yy]$ ]]; then
    echo "🔄 Rebuilding .env from Heroku staging..."
    {
        ...
        echo "ENCRYPTION_KEY=$(heroku config:get ENCRYPTION_KEY -a mwap-staging)"
        echo "JWT_SECRET=$(heroku config:get JWT_SECRET -a mwap-staging)"
        echo "SESSION_SECRET=$(heroku config:get SESSION_SECRET -a mwap-staging)"
    } > "$ENV_FILE"
...
if grep -q "DISABLE_API_V1=false" "$ENV_FILE"; then
  echo "⚠️ WARNING: Legacy API (v1) is still enabled in .env"
fi
